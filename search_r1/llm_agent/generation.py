...
class LLMGenerationManager:
    def __init__(
        ...
        self.law_article_retrieval = Legal_Retrieval()
        ...
        for i, (action, active) in enumerate(zip(cur_actions, active_mask)):
            ...
            if not active:
                ...
                if action == 'answer':
                    ...
                    next_obs.append(f'\n我之前的一些行为是无效的。\
                                    如果我想搜索, 我应该将问题放在<search>和</search>中间。\
                                    如果我想给出最后的回答，我应该将最终答案放在<answer>和</answer>中间，我再重新尝试一次.\n')
                    dones.append(0)
                    valid_action.append(0)
                    is_search.append(0)
    ...
    def _batch_search(self, queries):
        ...
        ################搜索法条使用#########################
        # print('queries = ', queries)
        # print('搜索法条 = ', self.law_article_retrieval.search_law_articles(queries))
        return self.law_article_retrieval.search_law_articles(queries)
        #########################################
    ...
    def _passages2string(self, retrieval_result):
        ...
        for idx in retrieval_result:
            # print('idx = ', idx)
            # print('doc_item = ', doc_item)
            try:
                fatiao_mingcheng = idx['law_name']
                fatiao_bianhao = idx['title']
                fatiao_neirong = idx['content']
                fatiao_neirong = fatiao_neirong.replace('\n', '')
                format_reference += f"法条名称: {fatiao_mingcheng}{fatiao_bianhao}内容：{fatiao_neirong}\n"
            except:
                # print('量刑指导')
                format_reference = idx
